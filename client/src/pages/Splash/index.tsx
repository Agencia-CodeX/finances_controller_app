...
export function Splash() {
  ...
  const diamondStartStyle = useAnimatedStyle(() => {
    return {
      ...
      transform: [
        {
          translateX: interpolate(splashAnimation.value, [0, 25], [0, -30]),
        },
      ],
    };
  });
  ...
  const diamondEndStyle = useAnimatedStyle(() => {
    return {
      ...
      transform: [
        {
          translateX: interpolate(splashAnimation.value, [0, 50], [-50, -50]),
        },
      ],
    };
  });
  ...
  const QFinance = useAnimatedStyle(() => {
    return {
      opacity: interpolate(splashAnimation.value, [0, 30, 50], [0, 0.1, 1]),
      transform: [
        {
          translateX: interpolate(splashAnimation.value, [0, 50], [-50, 40]),
        },
        {
          translateY: interpolate(splashAnimation.value, [0, 50], [6, 6]),
        },
      ],
    };
  });
  ...
  return (
    <Container>
      <Animated.View style={[diamondStartStyle, { position: "absolute" }]}>
